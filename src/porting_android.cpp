...
bool setSystemPaths()
{
	// Set user and share paths
	{
		...
		jobject result = jnienv->CallObjectMethod(activityObject, getUserDataPath);
	...
	// Set cache path
	{
		...
		jobject result = jnienv->CallObjectMethod(activityObject, getCachePath);
...
void showTextInputDialog(const std::string &hint, const std::string &current, int editType)
{
	...
	jnienv->CallVoidMethod(activityObject, showdialog,
...
void showComboBoxDialog(const std::string optionList[], s32 listSize, s32 selectedIdx)
{
	...
	jnienv->CallVoidMethod(activityObject, showdialog, jOptionList,
			jselectedIdx);
}
...
void openURIAndroid(const char *url)
{
	...
	jnienv->CallVoidMethod(activityObject, url_open, jurl);
}
...
AndroidDialogType getLastInputDialogType()
{
	...
	int dialogType = jnienv->CallIntMethod(activityObject, lastdialogtype);
...
AndroidDialogState getInputDialogState()
{
	...
	int dialogState = jnienv->CallIntMethod(activityObject, inputdialogstate);
...
std::string getInputDialogMessage()
{
	...
	jobject result = jnienv->CallObjectMethod(activityObject,
			dialogvalue);
...
int getInputDialogSelection()
{
	...
	return jnienv->CallIntMethod(activityObject, dialogvalue);
}
...
float getDisplayDensity()
{
	...
	if (firstrun) {
		...
		value = jnienv->CallFloatMethod(activityObject, getDensity);
		firstrun = false;
	}
...
v2u32 getDisplaySize()
{
	...
	if (firstrun) {
		...
		retval.X = jnienv->CallIntMethod(activityObject,
				getDisplayWidth);
		...
		retval.Y = jnienv->CallIntMethod(activityObject,
				getDisplayHeight);
