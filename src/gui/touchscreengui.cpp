		...
		const recti &button_rect, ISimpleTextureSource *tsrc, video::IVideoDriver *driver)
{
	...
	if (texture) {
		...
		if (g_settings->getBool("gui_scaling_filter")) {
			recti txr_rect = recti(0, 0, button_rect.getWidth(), button_rect.getHeight());
		...
		m_texturesource(tsrc)
{
	...
	// init crunch button
	addButton(drop_id, button_image_names[drop_id],
	...
	// init zoom button
	addButton(camera_id, button_image_names[camera_id],
