	...
	m_tsrc(tsrc)
{
	...
	m_vertical_image_padding = 4.0f * RenderingEngine::getDisplayDensity();
	...
	for (s32 j = 0; j < colcount; ++j) {
		...
		if (columntype == COLUMN_TYPE_TEXT) {
			...
			for (s32 i = 0; i < rowcount; ++i) {
				...
				row->content_width = 0;
				if (image) {
					// Fit into row (with some vertical padding)
					s32 rowheight = m_rowheight - m_vertical_image_padding;
					row->image_scale = (f32)rowheight / (f32)image->getOriginalSize().Height;
