...
vec4 getShadowColor(sampler2D shadowsampler, vec2 smTexCoord, float realDistance)
{
	...
	float samples = (1 + 1 * (SOFTSHADOWRADIUS > 1.0 ? 1.0 : 0.0)) * PCFSAMPLES; // scale max samples for the soft shadows
	samples = floor(clamp(pow(4.0 * radius + 1.0, 2.0), 1.0, float(samples)));
