...
bool CIrrDeviceSDL::run()
{
	...
	while (!Close && wrap_PollEvent(&SDL_event)) {
		...
		case SDL_EVENT_KEY_UP: {
			...
			if (SDL_TextInputActive(Window) && !keyIsKnownSpecial(key) && (SDL_event.key.mod & SDL_KMOD_CTRL) == 0)
				break;
			...
			irrevent.KeyInput.Char = findCharToPassToIrrlicht(SDL_event.key.key, key,
					(SDL_event.key.mod & SDL_KMOD_NUM) != 0);
...
bool CIrrDeviceSDL::setWindowIcon(const video::IImage *img)
{
	...
	SDL_Surface *surface = SDL_CreateSurface(width, height, SDL_GetPixelFormatEnumForMasks(
...
video::ECOLOR_FORMAT CIrrDeviceSDL::getColorFormat() const
{
	if (Window) {
		...
		if (surface->format->bits_per_pixel == 16) {
